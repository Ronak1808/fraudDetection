...
class GCN(torch.nn.Module):
    def __init__(self, input_size=8, hidden_size=16, lstm_hidden_size=32, output_size=2):
        super().__init__()
        ...
        self.conv2 = GCNConv(lstm_hidden_size, output_size)
    ...
    def forward(self, data):
        ...
        # LSTM layer
        # Assuming x has shape (batch_size, num_nodes, hidden_size)
        x, _ = self.lstm(x)

        # Second GCN layer
        output = self.conv2(x.view(-1, x.size(-1)), edge_index)  # Reshape x before passing to conv2
...
def train_node_classifier(model, graph, optimizer, criterion, n_epochs=200):
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch <=  10:
            ...
            print(f'Epoch : {epoch}, Train loss : {loss:.3f}, Acc : {acc:.3f}')
        elif epoch % 10 == 0:
            acc = evaluate(model, graph, graph.test_mask)
            print(f'Epoch : {9 + epoch/10}, Train loss : {loss:.3f}, Acc : {acc:.3f}')
...
gcn = train_node_classifier(gcn, graph, optimizer, criterion)
